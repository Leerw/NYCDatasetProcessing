...
def process( startyear  = 2015,
    ...
    for (year, month) in dates:
        ...
        if restart and year == startyear and month == startmonth:
            ...
            dates = utils.get_timeslots(year, month, (year + 1) if month == 12 else year, 1 if month == 12 else (month + 1), n)
            dates = np.asarray(dates)
            np.savez_compressed(save_filename_date + "-data.npz", data=vdata, date=dates, errors = np.array([invalid_count, unparsable_count]))
